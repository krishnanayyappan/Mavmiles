 ...
 */
@WebServlet("/EditMyProfileModel")
public class EditMyProfileModel extends HttpServlet {
	...
	public void setProfile(String name2, String loginid2,String password2, String email2,
			String dl_number2, String dl_expiry2, String phone2) {
		setname(name2);
		setloginid(loginid2);
	...
	public void validateProfile(EditMyProfileModel userProfile, EditMyProfileErrorMsgs UPerrorMsgs) {
		UPerrorMsgs.setNameerror(validateName(userProfile.getname()));
		UPerrorMsgs.setloginIDerror(validateloginID(userProfile.getloginid()));
		UPerrorMsgs.setpassworderror(validatePassword(userProfile.getpassword()));
		
		
		
		UPerrorMsgs.setemailError(validateemail(userProfile.getemail()));
		UPerrorMsgs.setdl_numberError(validatedrivingLicense(userProfile.getdl_number()));
		try {
			UPerrorMsgs.setdl_expiryError(validatedlexp(userProfile.getdl_expiry()));
		} catch (java.text.ParseException e) {
			e.printStackTrace();
		}
		UPerrorMsgs.setphoneError(validatecontact(userProfile.getphone()));

		
		UPerrorMsgs.setErrorMsg();
	}
